...
product_info = urcos_product_crawler(product_code =6879 )
...
author_url = urcos_product_userComment(product_code = 6879)


# 2.2 取得所有user comment該產品的資料####

length(author_url$author_url)
...
urcos_product_userCommentInfo = function(i = 1, crawl_author_other_post =F ,
  ...
  if (crawl_author_other_post) { # 爬鎖定的作者其他文章
    ...
    product_code = doc_product_comment %>%
      html_nodes(xpath = '/html/body/div[5]/div[2]/div[1]/div[1]/ol/li[4]/a') %>% 
      html_attr('href') %>% parse_number()
...
author_info =urcos_product_userCommentInfo(i = 1)

author_product_comment = cbind(author_info, product_info)
...
urcos_userPostsAll_content = function(pageall=pageall, i=1){
  ...
  return(product_comment_n_info_df)
}

# 最終極
# 輸入產品代號，就可以
urcos_crawler = function(product_code  = 84630){
  
  # 抓消費者個人網址 與 產品評論網址
  author_url = urcos_product_userComment(product_code = product_code)
  
  #author_url = author_url[1:2,]
  ...
  tryCatch({
    ...
    for (i in 1:length(author_url$author_url)) {
      ...
      product_comment_n_info_df = urcos_userPostsAll_content(pageall=pageall, i=i)
...
# 範例：
all_df = urcos_crawler(product_code  = 你的產品號碼)
all_df = all_df[[1]]
